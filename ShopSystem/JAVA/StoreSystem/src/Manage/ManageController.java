...
public class ManageController implements Initializable{
    ...
    @FXML
    private TableColumn<Supplier, String> CLM_sWebsite;
    @FXML
    ...
    @FXML
    private TableColumn<Item, String> CLM_iSize;
    @FXML
    private TableColumn<Item, String> CLM_iColor;
    @FXML
    ...
    @FXML
    private TextField TF_iSize;
    @FXML
    private TextField TF_iColor;
    @FXML
    ...
    public void addNewEmployee(ActionEvent event) {
    	try {
    		if (isAllFieldsFilledForDeleteEmployee()) {
				Driver.addNewEmployeeToTheDatabase(TF_name.getText(), Integer.parseInt(TF_ssn.getText()), Integer.parseInt(TF_phoneNumber.getText()),
    	...
    	try {
	    	...
	    	if (eIndex > -1) {
	    		if (isAllFieldsFilledForUpdateEmployee()) {
		    		...
		    		Driver.updateEmployee(id, TF_name.getText(), Integer.parseInt(TF_ssn.getText()), Integer.parseInt(TF_phoneNumber.getText()), 
							TF_address.getText(), Double.parseDouble(TF_salary.getText()), TF_startDate.getValue().toString(), endDate);
...
//			e.printStackTrace();
    ...
    private void updateSupplierTable() throws SQLException {
		...
		CLM_sWebsite.setCellValueFactory(new PropertyValueFactory<Supplier, String>("website"));
        ...
        TF_sSearch.textProperty().addListener((observable, oldValue, newValue) -> {
			filteredData.setPredicate(supplier -> {
				...
				else if (supplier.getWebsite().indexOf(lowerCaseFilter)!=-1) {
				     return true;
    ...
    private void getSelectedSupplier() {
    	...
    	if (sIndex > -1) {
    		...
    		TF_sWebsite.setText(CLM_sWebsite.getCellData(sIndex));
    ...
    public void addNewSupplier(ActionEvent event) {
    	try {
    		if (TF_sName.getText() != "" && TF_sPhoneNumber.getText() != "") {
    			Driver.addNewSupplierToTheDatabase(TF_sName.getText(), Integer.parseInt(TF_sPhoneNumber.getText()), TF_sWebsite.getText(), 
    					TF_sInfo.getText(), TF_sAddress.getText());
    			updateSupplierTable();
    	...
    	try {
    		...
    		if (sIndex > -1) {
    			if (TF_sName.getText() != "" && TF_sPhoneNumber.getText() != "") { 
    				...
    				Driver.updateSupplier(id, TF_sName.getText(), Integer.parseInt(TF_sPhoneNumber.getText()), TF_sWebsite.getText(), TF_sInfo.getText(), TF_sAddress.getText());
					updateSupplierTable();
					clearSuppliersTextFields(event);
    			}
    			else
...
//			e.printStackTrace();
    ...
    private void updateItemsTable() throws SQLException {
		...
		CLM_iSize.setCellValueFactory(new PropertyValueFactory<Item, String>("size"));
		CLM_iColor.setCellValueFactory(new PropertyValueFactory<Item, String>("color"));
        ...
        TF_iSearch.textProperty().addListener((observable, oldValue, newValue) -> {
			filteredData.setPredicate(item -> {
				...
				if (String.valueOf(item.getId()).indexOf(lowerCaseFilter)!=-1) {
					...
					return true; // Filter matches last name.
				} else if (item.getSize().indexOf(lowerCaseFilter)!=-1) {
					return true; // Filter matches last name.
				}  else if (item.getColor().indexOf(lowerCaseFilter)!=-1) {
					return true; // Filter matches last name.
				} 
				else  
				    return false; // Does not match.
    ...
    private void getSelectedItem() {
    	...
    	if (iIndex > -1) {
    		...
    		TF_iSize.setText(CLM_iSize.getCellData(iIndex));
    		TF_iColor.setText(CLM_iColor.getCellData(iIndex));
